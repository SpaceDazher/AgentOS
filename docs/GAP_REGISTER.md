...
| Tamper evidence | SHA-256 chain + in-DB anchor + external mirror `audit_anchor.head`; pack build fails loudly on tamper/inconsistent ACCEPTED; `anchor-export`/`anchor-verify` (`agentos.anchor-export/v1`) for point exports; `anchor-mirror` + `scripts/anchor-mirror-task.ps1` maintain an idempotent off-host mirror (immutable bundles, append-only history) | mirror target location/transport is operator-owned; no notarized timestamping yet | point `anchor-mirror` at a git repo/object store; optional notarization |
