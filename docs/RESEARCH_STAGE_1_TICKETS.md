...
- **Status:** `PASS_WITH_LIMITS` (canonical revision 2; placement
  `OFFLINE_ANALYTICS`, research execution `pass_with_limits` with limits)
- **Result:** operator review `1A 2A 3A 4A 5A 6A 7A 8A 9A 10A`; responsibility
  analytics runs offline over immutable audit/lineage export only. Annotations
  are non-authoritative (`authority=false`), never read by the Gateway, and
  legal/moral blame is `OUT_OF_SCOPE`. Evidence: 48 scenarios x 3 placements x
  3 seeds x 2 executors = 864 observations; R1-R14 zero; gateway-owned
  authorization, construction-oracle agreement, and abstention correctness are
  100%; false/missed attribution are zero; probes A-P pass; replay is
  byte-identical. Sensitivity uses 259 deterministic vectors (model counts and
  artifact bytes only, no wall-clock), selects placement A, and has zero flips.
- **Priority:** `P2`
- **Wave:** `W4`
- **Owner:** `formal`
