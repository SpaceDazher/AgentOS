...
Статус: черновик v1 (раунд 3). Источники: `research/sources/*.md`, базовый документ.
...
| `acts_for` | AgentRuntimeInstance → HumanUser \| SharedWorkspace | действует от имени (subject) через grant | RFC 8693 actor [B4]; prov:actedOnBehalfOf [F8] |
...
subject (HumanUser XOR SharedWorkspace XOR platform-operations) и одного актора —
...
| Задача/Run | Task lifecycle | Task (2025-11-25) [A6] | prov:Activity | — |
...
hubs:DelegationGrantShape a sh:NodeShape ;
  ...
  sh:closed true ;
  sh:property [ sh:path hubs:subject ; sh:minCount 1 ; sh:maxCount 1 ] ;
  sh:property [ sh:path hubs:actor ; sh:minCount 1 ; sh:maxCount 1 ] ;
  ...
  sh:property [ sh:path hubs:budget ; sh:maxCount 1 ] .
```
...
- `Role`-схемы per workspace — канонический список viewer/editor/contributor/indexer/curator
  ...
  [D9][D17], но без role-explosion: роли привязаны к workspace-типам, не к парам «пользователь×ресурс» [C1].
- `Commitment` (debtor, creditor, goal, deadline, discharge/cancel условия) — публично проверяемый
  социальный объект для межагентных обещаний [D18][D12]; нарушение commitment → machine-checkable
...
- Q3: гранулярность Evidence (документ vs span vs digest) — предложить трёхуровневую модель в
  60_mathematical_model.md (вес доказательства).
