...
Статус: черновик v1 (раунд 5). Трассировка: каждая фича несёт `{H…}` (гипотезы базового
...
| ID | Фича | Описание | AC | Приоритет |
|---|---|---|---|---|
| F-3.1 | DelegationGrant issuance | UI выдачи гранта: субъект, актор, действия, ресурсы, TTL, бюджет | derive() не может выпустить грант шире родительского (матмодель §3.3) | P0 |
...
| F-3.5 | Quotas/budgets | token bucket per agent/run; max fanout/depth | превышение бюджета → отказ/approval, не бесконечный цикл [J3] | P0 |
...
| F-5.6 | MCP Tasks alignment | durable task objects со status/resume как в MCP 2025-11-25 [A6] | task id недоступен без authorization context | P1 |
...
| F-7.2 | Evaluation service | независимые оценщики (heterogeneous); вердикты attests | disagreement > порога эскалирует человеку | P1 |
...
| F-7.6 | Promotion thresholds | Beta-постериор ≥ порога; n_src≥2 (матмодель §5) | подделка одним источником не промоутит claim | P1 |

## EP-08 · Аудит, провенанс, экспорт {H15} [I8,I15,F7,F8,H3,H4,H6,H16,H17]

| ID | Фича | Описание | AC | Приоритет |
|---|---|---|---|---|
| F-8.1 | Authoritative audit log | append-only события решений/эффектов + Merkle-цепочка | proof включения любого event проверяется; потеря событий при crash=0 (§9.2.8–9) | P0 |
| F-8.2 | Transactional outbox | состояние+событие атомарно; CloudEvents envelope | ровно один эффект на decision_id (idempotency) | P0 |
...
## EP-10 · UX видимости и ментальная модель {H15} [K1,K3,K4,K7,K10,K11,K16→G16,K6,K9]
...
| F-11.3 | Deterministic simulation harness | сидируемый scheduler + fault injection (FoundationDB-стиль) | 10⁶ операций без нарушений INV1–5/SAF | P1 |
| F-11.4 | Benchmark suite | seed §5.3 envelope автоматизирован | p95 authz<20ms; API<250ms; 40 concurrent runs; 1000 adversarial checks=0 | P0 |
