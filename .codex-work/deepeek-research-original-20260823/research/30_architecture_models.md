...
Статус: черновик v1 (раунд 6). Уровни: контекст → контейнеры → компоненты → данные →
...
SLO: p95 < 20 ms локально; revoke-class кэш TTL = 0
...
saga: каждый side-effect имеет compensation (длинные действия) [H5]
fan-out: F_max ≤ 10, depth ≤ 3 на старте [E16]; budget token-bucket per run [J3]
crash: восстановление replay'ем event history [H8][H9]; ровно-один-эффект по decision_id [H6]
```

### 3.3 Knowledge Gate pipeline

```text
proposed(из Message/Artifact) → сбор Evidence → Evaluation(n_src ≥ 2, гетерогенно)
...
Merkle-цепочка блоков аудита (RFC 6962-стиль [I15]); relay публикует подписчикам at-least-once
```
...
grants(id PK jti, subject_user FK, subject_workspace FK NULL, actor_installation FK,
       actions TEXT[], resources TEXT[], constraints JSONB, expires_at, budget_left,
...
audit_events(seq PK, ts, decision_id, envelope JSONB, prev_hash, hash); -- append-only
outbox(seq PK, envelope JSONB, published_at NULL);                      -- transactional outbox [H4]
...
```text
POST /grants                 -> выдать грант (owner authn; exact scope)
...
ключи DPoP/mTLS агентов — на стороне установок [B6][B7]. Recovery targets seed §5.3:
process crash < 60 c (supervision + PITR), 0 потерянных audit events (outbox).
