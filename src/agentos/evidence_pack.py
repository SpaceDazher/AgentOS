...
def build(db, root_dir: str | Path, goal_id: str) -> dict:
    ...
    events = rows("SELECT seq, ts, actor, event_type, payload_json, prev_event_sha256"
                  " FROM audit_event WHERE goal_id=? ORDER BY seq", (goal_id,))
    # verify chain over this goal's events
    chain_ok = True
    prev = None
    from .ids import sha256_text as sh, canonical_json as cj
    for ev in events:
        if ev["prev_event_sha256"] != prev:
            chain_ok = False
            break
        body = cj({"goal_id": goal_id, "actor": ev["actor"],
                   "event_type": ev["event_type"],
                   "payload": json.loads(ev["payload_json"])})
        prev = sh((ev["prev_event_sha256"] or "") + "|" + body)
