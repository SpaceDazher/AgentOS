"""Publish S1-016 canonical evidence from trusted local SQLite state."""
...
def canonical_dependencies() -> list[dict]:
    ...
    if {item["ticket_id"] for item in proofs} != {"S1-003", "S1-007"}:
...
def verify_binding(series: dict, evaluation: dict, pack: dict) -> None:
    ...
    if evaluation.get("result") == "pass":
        raise ValueError("PASS is out of scope for S1-016")
...
def _publish(args) -> int:
    publisher = load_module(HERE / "make_bundle.py", "s1016_publication_gate")
    ...
    sensitivity_doc = publisher._read_json(HERE / "results" / "sensitivity.json", "sensitivity")
    blockers, verdict = publisher.derive_verdict(metrics, comparison, sensitivity_doc,
                                                 False, None)
    if blockers:
    ...
    with sqlite3.connect(db_file.as_uri() + "?mode=ro", uri=True) as conn:
        ...
        series_row = conn.execute(
            ...
            "ORDER BY revision DESC LIMIT 1", ("S1-016",)).fetchone()
        if series_row is None:
            raise ValueError("no canonical S1-016 series")
        ...
        if evaluation_row is None:
            raise ValueError("no canonical S1-016 evaluation")
    ...
    tests = sorted((REPO / "tests").glob("test_s1_016*.py"))
    ...
    payload = {
        "schema": "agentos.s1-016.ticket-evidence/v1",
        "ticket_id": "S1-016",
        "series": series,
        ...
        "closure": {"design_decision": candidate["design_decision"],
                    "substance_leader": candidate.get("substance_leader"),
                    "sensitivity_flips": candidate.get("sensitivity_flips", 0),
                    "operator_review_n": candidate.get("operator_review_n", 1),
