...
Phase B closure tool for S1-012 (mirrors the S1-010/S1-011 procedure):

- re-runs the REAL publication basis (make_bundle.derive_verdict, which
  itself recomputes the whole pipeline from the tracked raw cells via
  compare_runs and crosschecks the saved merged artifacts) before anything
  is published;
- verifies the tracked S1-001/S1-003/S1-011 dependency records equal the
  latest canonical DB rows and that their artifact chains recompute from
  disk;
- reads the canonical DB read-only and refuses a stale bundle/pack/
  evaluation binding;
...
def main() -> int:
    ...
    # --- gate 1: the real S1-012 publication basis must pass again --------
    import importlib.util
    spec = importlib.util.spec_from_file_location(
        "s1012_make_bundle_gate", HERE / "make_bundle.py")
    gate = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(gate)
    gate.HERE = HERE
    gate.RESULTS = HERE / "results"
    blockers, facts = gate.derive_verdict(here=HERE, results=HERE / "results")
    if blockers:
        raise ValueError("publication basis refused: " + "; ".join(blockers))
    ...
    with sqlite3.connect((db_root / "agentos.db").as_uri() + "?mode=ro",
        ...
        series_row = conn.execute(
            "SELECT * FROM research_series WHERE research_key=? "
            "ORDER BY revision DESC LIMIT 1", ("S1-012",)).fetchone()
        if series_row is None:
            raise ValueError("no canonical S1-012 series")
    ...
    registry = candidate.get("tracked_artifacts", {})
    payload = {"schema": "agentos.s1-012.ticket-evidence/v1",
               "ticket_id": "S1-012",
               "series": series, "evaluation": evaluation,
               "canonical_dependencies": dependencies,
               "canonical_pack": canonical_pack,
               "tracked_artifacts": registry}
