"""S1-018 process-separated replay (Run A / Run B, 864 observations).
...
executor ID, nonce and output roots. Canonical decisions, digests,
counters, leakage reports and probe outcomes must match. Same-host replay
is called replay, never an external audit.
"""
...
def _writable_tmp() -> Path:
    for candidate in (os.environ.get("TMP"), os.environ.get("TEMP"),
        ...
        if candidate and Path(candidate).is_dir():
            try:
                probe = Path(candidate) / ".s1018-write-test"
...
def run_once(ticket: Path, work: Path, tag: str, executor: str) -> dict[str, Any]:
    ...
    env = dict(os.environ)
    env["S1018_EXECUTOR"] = executor
    env["S1018_NONCE"] = nonce
    ...
    if metrics.get("schema") != "agentos.s1-018.metrics/v1":
    ...
    decisions = sorted((c.get("scenario_id") or c.get("case_id"),
                        c.get("placement") or c.get("architecture"),
                        c.get("seed"), c.get("status"),
                        c.get("decision") or c.get("verdict"))
                       for c in cores)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="S1-018 replay")
    parser.add_argument("--ticket", required=True)
    parser.add_argument("--out", required=True)
    args = parser.parse_args(argv)
    ticket = Path(args.ticket).resolve()
    output = Path(args.out).resolve()
    work = Path(tempfile.mkdtemp(prefix="s1018-repl-", dir=str(_writable_tmp())))
    try:
        ...
        content_match = first_cores == second_cores
        digests = {
            ...
            "metrics_counters": {
                "a": _digest(first["metrics"]["pc_violations"]),
                "b": _digest(second["metrics"]["pc_violations"]),
                "match": first["metrics"]["pc_violations"] ==
                second["metrics"]["pc_violations"]},
            "probes": {"a": _digest(first["probes"]), "b": _digest(second["probes"]),
                       "match": first["probes"] == second["probes"]},
            "observation_content": {
                "a": _digest(first_cores), "b": _digest(second_cores),
                "match": content_match},
        }
        manifests_match = (
        ...
        doc = {
            "schema": "agentos.s1-018.comparison/v1",
            ...
            "matrix": "48 cases x 3 architectures x 3 seeds x 2 executors = 864 observations",
