"""Publish S1-018 canonical evidence from trusted local SQLite state."""
...
def canonical_dependencies() -> list[dict]:
    ...
    if {item["ticket_id"] for item in proofs} != {"S1-007", "S1-008", "S1-009"}:
...
def verify_binding(series: dict, evaluation: dict, pack: dict) -> None:
    ...
    if evaluation.get("result") == "pass":
        raise ValueError("PASS is out of scope for S1-018")
...
def _publish(args) -> int:
    publisher = load_module(HERE / "make_bundle.py", "s1018_publication_gate")
    ...
    present, answers, _ = publisher.verify_operator_decision(HERE)
    if not present:
    ...
    with sqlite3.connect(db_file.as_uri() + "?mode=ro", uri=True) as conn:
        ...
        series_row = conn.execute(
            ...
            "ORDER BY revision DESC LIMIT 1", ("S1-018",)).fetchone()
        if series_row is None:
            raise ValueError("no canonical S1-018 series")
        ...
        if evaluation_row is None:
            raise ValueError("no canonical S1-018 evaluation")
    ...
    if candidate.get("status") not in ("CLOSED_WITH_LIMITS", "CLOSED_INCONCLUSIVE") or \
            candidate.get("hardware_tee_evidence") != "NOT_MEASURED" or \
    ...
    tests = sorted((REPO / "tests").glob("test_s1_018*.py"))
    ...
    payload = {
        "schema": "agentos.s1-018.ticket-evidence/v1",
        "ticket_id": "S1-018",
        "series": series,
