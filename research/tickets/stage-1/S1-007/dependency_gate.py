"""S1-007 — preflight dependency gate (S1-003, S1-005).
...
- goal/campaign/evaluation ids and the artifact chain hash agree between
  the record and the canonical DB;
- chain_fresh=true and latest_evaluation_valid=true are recorded;
- the docs status matches the canonical verdict.

Fails closed (exit 1) and stores the machine-readable result in
dependency-gate.json on success.
"""
...
def validate_dependency_binding(rec: dict, pack: dict, db_eval,
    ...
    if len(matches) != 1:
        ...
        for key in ("result", "artifact_chain_hash", "campaign_id", "goal_id"):
            if packed_eval.get(key) != rec.get(key):
    ...
    if db_eval is None:
        ...
        for key in ("id", "result", "artifact_chain_hash", "campaign_id",
                    "goal_id"):
            expected = rec.get("evaluation_id" if key == "id" else key)
            if db_eval[key] != expected:
...
def check(ticket: str) -> dict:
    ...
    return {
        ...
        "pack_path": rec["evidence_pack"]["path"],
        "pack_sha256": rec["evidence_pack"]["sha256"],
        "payload_sha256": rec["evidence_pack"]["payload_sha256"],
        "chain_fresh": rec["evidence_pack"].get("chain_fresh") is True,
        "latest_evaluation_valid":
            rec["evidence_pack"].get("latest_evaluation_valid") is True,
        "problems": problems,
...
def main() -> int:
    results = [check(t) for t in ("S1-003", "S1-005")]
    doc = {
        "schema": "agentos.s1-007.dependency-gate/v1",
        "ticket": "S1-007",
    ...
    if not doc["all_proven"]:
        ...
        print(f"BLOCKED: dependency evidence not proven for {failed}",
              file=sys.stderr)
        return 1
    print("DEPENDENCY GATE: S1-003 and S1-005 PROVEN", file=sys.stderr)
    return 0
