...
Статус: v2 (независимый аудит, раунд 13). Решение по нумерации: **перенумерация в S### отменена** — сохранены
...
| Серия SV (обзоры) | 3 |
| **Всего позиций в реестре** | **247** (253 строки таблицы: 6 — указатели дубликатов; 246 валидных уникальных после исключения I4) |
| **Ядро цитирования модельных артефактов (★)** | **118** — курируемый слой внутри исходного диапазона 100–200; полный набор превышает его (см. отклонение в `00_research_plan.md`) |
...
| L13 | C8+C10 | Cedar OOPSLA-пейпер + docs (сборный дубль) |
...
|---|---|---|
| v | URL подтверждён поиском как есть | 44 |
| c | подтверждён с канонической правкой URL/DOI либо явно заменён корректной работой | 26 |
| x | не подтвердился ×неск. попыток — ручная проверка | 0 |
| x-исключён | неверный идентификатор | 1 (I4) |
| u | вне объёма верификации | 176 |
...
MCP актуальная ревизия 2026-07-28 (A4), Tasks вынесены в extension (A6); шесть прежних
`x` разрешены каноническими записями/явными заменами (C1,C8,C9,D12,L17,J4).
...
| A5 | MCP Authorization | c | ★ | офиц. ревизия 2026-07-28 |
| A6 | MCP Tasks extension | c | ★ | вынесено из core 2025-11-25 в официальный extension |
...
| ID | Источник | Вер. | Ядро | Правка/примечание |
|---|---|---|---|---|
| C1 | RBAC96 Sandhu et al. | c | ★ | DOI 10.1109/2.485845; role-explosion — дизайн-вывод |
...
| C7 | OpenFGA docs | u | | |
| C8 | Cedar OOPSLA'24 | c | ★ | DOI 10.1145/3649835; venue исправлен |
| C9 | How We Built Cedar FSE'24 | c | | явная замена несуществующей SOSP'23 записи; DOI 10.1145/3663529.3663854 |
...
| D10 | ISLANDER | u | | |
| D11 | STEAM Tambe | u | | |
| D12 | Teamwork Cohen&Levesque, Noûs | c | | DOI 10.2307/2216075; venue исправлен |
...
| E17 | Anthropic Building Effective Agents (канон дубля A15) | u | | восстановлен: ранее обе стороны duplicate-пары были ошибочно сняты |
| E18 | τ-bench | u | | |
| E19 | GAIA | u | | |
...
| L17 | Fong & Siahaan ReBAC SACMAT'11 | c | | исходный DOI был чужим; DOI 10.1145/1998441.1998450 |
...
| J4 | Netflix adaptive concurrency | c | | официальный пост переехал на netflixtechblog.medium.com |
...
1. Шесть прежних `x`-строк проверены браузером в раунде 13; C9 и L17 заменены с явным
   сохранением provenance исходной ошибки. Неразрешённых `x` больше нет.
